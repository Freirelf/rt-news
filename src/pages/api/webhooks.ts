...
import { saveSubscripton } from "./_lib/manageSubscription";
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === 'POST') {
    ...
    try {
      ...
      return res.status(480).send(`Webhook error: ${err.message}`);
    }
      ...
      try {
        switch (type) {
          ...
          case 'checkout.session.completed':
            ...
            break;
          default:
            throw new Error('Unhandelsd event')
        }
      } catch (err) {
